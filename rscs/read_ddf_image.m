...
rootname = "E3650_csi_cor_sum_comb_phased";
displayflag = 1;
complexformat = 0;

% nargin returns the number of function input arguments given in the call to the currently executing function. 
% Use this syntax in the body of a function only. Default input is 1 arguments, complex situation is more 
% than 1 input
% -------------------------------------------------------------------------------------------------------------
...
% converts root in a char array
% -------------------------------------------------------------------------------------------------------------
rootname = char(rootname);
unzipped_flag = 0;

% If we you want the script to be verbose (clutter screen or not)
% -------------------------------------------------------------------------------------------------------------
...
if (complexformat == 0)
  temp_data = fread(datafile, prod(output.ddf.npix)*output.ddf.specpoints*2, 'float');
  
  disp(temp_data(43545))
  ...
  output.img = complex(temp_data(1:2:length(temp_data)), temp_data(2:2:length(temp_data)));
  %{
  >>> disp(output.img)
  ...
    0.0091 - 0.0061i
  ...
  %}
  %before = output.img;
  output.img = reshape(output.img, [output.ddf.specpoints output.ddf.npix]);
  %after = output.img;
  
  %disp(after(1,10,11,1))
  %{
  y = output.img(:,1,1,1);
  count = 0;
  for i = 1:2048
  ...
  disp("Number of Non-zero y values")
  disp(count)
  disp("----------------------------------------------")
